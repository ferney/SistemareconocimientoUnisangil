...
class CaptureImage(QWidget):
    def __init__(self):
        ...
        self.setGeometry(100, 50, int(1920 / 2), int(1080 / 2) + 120)
        ...
        self.button.setGeometry(360, 560, 140, 70)
    ...
    def start_capture(self):
        name = self.entry_name.text().strip()+ ", "+self.entry_dni.text().strip()
        if name == "":
            QMessageBox.critical(self, "Error", "Por favor ingresa un nombre")
            return

        #check name exist or not
        if os.path.exists("names.txt"): #names.txt
